...
- **`config/overlays/garam-dev/` is the overlay for `admin@garam-dev`, and is the only overlay this repository holds.** It sets the three flags above through an appended argument, and it names the operator image that cluster already runs, so that applying it changes those three arguments and nothing else.
