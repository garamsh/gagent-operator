...
- **Whether the two intervals are ever an environment's.** `--garam-poll-interval` and `--garam-renewal-interval` have defaults and nothing sets them, so the base owns them by omission. Both are read against windows `garam` measures, so a cluster whose `garam` chose a different window would be what moves them; none has.
