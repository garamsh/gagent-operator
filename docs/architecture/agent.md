...
The Pod's group and the group-readable mode are [ADR 0006](adr/0006-credential-group.md), settled on issue #31 after the e2e layer measured what no layer below it could see: a container that drops root cannot read an owner-only file the kubelet wrote as root.
