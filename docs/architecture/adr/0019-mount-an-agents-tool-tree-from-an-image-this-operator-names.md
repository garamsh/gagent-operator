...
**Nothing here has been exercised against a cluster, and no image exists to exercise it with.** Issue #123 records that the tool binaries are gitignored build outputs of `make build-tools`, that neither `gagent` image copies them, and that no registry holds them; publishing one is blocked on issue #102. So `--agent-tools-image` names nothing until that lands, the e2e suite is not extended, and what has been shown here is what the operator builds and what a `restricted` namespace admits — not an agent that starts. The state issue #123 asks to verify against — an agent reporting `ready` and a turn that reaches `garam` — is not reached by this decision alone.
