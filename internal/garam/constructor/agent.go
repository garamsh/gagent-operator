...
// Agent constructs the agents garam assigned this operator.
//
...
func (a *Agent) Construct(ctx context.Context, agent garam.GRN, credential garam.AgentCredential) error {
	constructed, err := a.ensureAgent(ctx, agent)
	if err != nil {
		return err
	}
...
// one, and reports the GRN it was constructed from on it.
func (a *Agent) ensureAgent(ctx context.Context, agent garam.GRN) (*agentv1alpha1.Agent, error) {
	...
	// Reported rather than written in the spec: a GRN garam minted is not
	// something a user writes, and garam is where a claim is durable.
	//
	...
	patch, err := json.Marshal(map[string]any{"status": map[string]string{"agent": string(agent)}})
	if err != nil {
